...
###############################################################################################
# Generation of the r(t) function via shell command (function is written to file "r_of_t.gp") #
###############################################################################################
`cat sine_100_Hz.csv | tail -n +3 |awk 'BEGIN{firstDone=0; print "r(t) = \\"} {if ((NR%40) == 0) {if(firstDone) print " + \\"; split($0,a,","); printf("%f * k(t%s%f)", a[3], ((a[1]<0)? "+" : ""), -a[1]); firstDone=1}} END{printf("\n")}' > r_of_t.gp`

load("r_of_t.gp")


####################################################
# Reconstruction (Interpolation) via Linear Kernel #
####################################################
tAbs(t) = (t < 0)? -t : t
...
r(x) w l lc rgb 'red', \
"< cat sine_100_Hz.csv | tail -n +3 | awk '{if ((NR%40) == 0) print}'" u 1:3 w p pt 7 ps 0.75 lc rgb 'black'

pause -1


##################################
# Reconstruction via Sinc Kernel #
##################################
